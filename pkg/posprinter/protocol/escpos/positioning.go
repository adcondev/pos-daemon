...
func SetHorizontalTabPositions(n types.TabColumnNumber, k types.TabTotalPosition) ([]byte, error) {
	...
	for i, col := range n[:k-1] {
		if col == 0 {
			return nil, fmt.Errorf("la posición de tabulación %d no puede ser cero", i+1)
		}
		if col >= n[i+1] {
			return nil, fmt.Errorf("las posiciones de tabulación deben ser ascendentes: %d no es mayor que %d", col, n[i-1])
		}
	}
